...
export default function SetupPasswordPage() {
  ...
  useEffect(() => {
    ...
    const preloadEmail = async () => {
      ...
      if (!alive) return;
      if (!user?.email) {
        toast.error("Este enlace no es válido o expiró. Solicita una nueva invitación.");
        navigate("/login");
        return;
      }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setLoading(true);
    try {
      const response = await setPasswordForInvitedUser(values.email, values.password);
      if (response.error) {
        toast.error(response.error.message || "No se pudo configurar la contraseña.");
        return;
      }
      toast.success("Contraseña configurada. Ahora inicia sesión.");
      await logout();
      navigate("/login");
    } catch {
      ...
      <div className="flex-1 flex flex-col gap-8 lg:gap-12 justify-center items-center bg-soft-blue lg:col-span-8 px-4 py-12 sm:px-6 relative z-10">
        <div className="flex flex-col items-center gap-4 lg:hidden">
          <img
            ...
            alt={getAppName()}
            className="w-20 h-20 rounded-2xl shadow-lg bg-primary-blue p-2"
          />
        </div>
